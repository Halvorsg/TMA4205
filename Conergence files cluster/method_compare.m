...
norm_VC_smth=zeros(max_post_smth,max_pre_smth);
time_VC_smth=zeros(max_post_smth,max_pre_smth);
cnt = 0;
TotIter = (max_post_smth*smth_step)*(max_pre_smth*smth_step)*(loops+1);
for post_smth_cnt=1:smth_step:(max_post_smth*smth_step)
    for pre_smth_cnt=1:smth_step:(max_pre_smth*smth_step)
        ...
        for iterator=0:loops
            ...
            time_VC=(time_VC*iterator+time_VC_stamps_it)/(iterator+1);
            cnt = cnt+1;
            fprintf('Number done: %f \n', cnt)
            fprintf('Number left: %f \n', TotIter-cnt)
        end
        norm_VC_smth(post_smth_cnt,pre_smth_cnt)=norm_VC(end);
        time_VC_smth(post_smth_cnt,pre_smth_cnt)=time_VC(end);
    end
    save('norm_VC_smth','norm_VC_smth')
    save('time_VC_smth','time_VC_smth')
    disp(time_VC_smth)
end

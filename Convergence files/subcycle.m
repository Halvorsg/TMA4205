...
FLOPS = [FLOPS;FLOPS(end)+5*M*N*pre_s];
...
FLOPS = [FLOPS;FLOPS(end)+5*M*N*post_s];
...
% title('Post smoothing')
    ...
    %% Conjugate Gradient
    x0=[u0;v0];
    [x,r,cnt]=conjugate_gradient(Syst_mat,RHS,x0(:),10^-8,1000);
    FLOPS = [FLOPS;FLOPS(end)+5*M*N*cnt];
    norm1_r = [norm1_r;norm(r)];

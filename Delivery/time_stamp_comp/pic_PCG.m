...
norm1_r= norm(r);
[ zu1,zv1 ,~,A,SMP]=pic_subcycle(Syst_mat,RHS,1,u1(:),v1(:),M,N,3,3,4,norm1_r,A,SMP,flag);
flag = false;
z = [zu1;zv1];
 p = z; s = z'*r;
ZEROS = zeros(size(u1));
...
while sqrt(norm(s)) > tol && cnt < maxit
    ...
    x = x + alpha*p;
    r = r-alpha*Ap;
    [ zu1,zv1 ,~,A,SMP]=pic_subcycle(Syst_mat,r,1, ZEROS, ZEROS ,M,N,3,3,4,norm1_r,A,SMP,flag);
    z = [zu1;zv1];
    s_old = s;
    s = r'*z;
    beta = s/s_old;
